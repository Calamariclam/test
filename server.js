...
class io_circleMaster extends IO {
    constructor(body) {
        super(body);
        this.turnwise = 1;
    }
    ...
    think(input) {
        ...
        if (input.target != null && (input.alt || input.main)) {
            ...
            if (input.alt) {
                ...
                // Orbit point
                let dir = this.turnwise * target.direction + 1;
                goal = {
                    x: this.body.x + target.x - orbit * Math.cos(dir),
                    y: this.body.y + target.y - orbit * Math.sin(dir), 
                };
...
const levelers = [
    ...
    42, 44, 45, 60, 69, 75, 90, 105,120,135,
    150,165,180,195,210,211,212,213,214,215,
    225,240,255,270,285,300,315,330,345,360,
    375,390,403,404,405,406,407,408,409,410,
    ...
    live() {
        ...
        if (this.canShoot) {
            ...
            if (shootPermission || !this.waitToCycle) {
                if (this.cycle < 1) {
                    this.cycle += 1 / this.settings.reload / roomSpeed / ((this.calculator == 'fixed reload') ? 1 : sk.rld);
                } 
            } 
            // Firing routines
...
class Entity {
    ...
    define(set) {
        if (set.FELIPE != null) {
          this.felipe = set.FELIPE;
        }
        if (set.FELIPE_BULLET != null) {
          this.felipeBullet = set.FELIPE_BULLET;
        }
        if (set.TEAM != null) {
          this.team = set.TEAM;
        }
        ...
        if (set.tf != null) { 
            this.anicepairofsmashers = set.tf; 
        }
        if (set.SHAPE != null) {
        ...
        if (set.DIE_OF_HUNGER != null) {
          this.settings.diesOfHunger = set.DIE_OF_HUNGER;
        }
        if (set.FRIENDLY_FIRE != null) {
          this.settings.friendlyFire = set.FRIENDLY_FIRE;
        }
        if (set.RCTD != null) {
          this.settings.rightClickToDie = set.RCTD;
        }
        if (set.CHASE_BOOST != null) {
          this.settings.boostInLongChase = set.CHASE_BOOST;
        }
        if (set.BCR != null) {
          this.settings.chaseTimeStart = set.BCR;
        }
        if (set.EXTRA_UPGRADE_MESSAGE != null) {
          this.settings.specialMessage = set.EXTRA_UPGRADE_MESSAGE;
          this.settings.hasSpecialMessage = true;
        }
        if (set.EXTRA_UPGRADE_MESSAGE == null) {
          this.settings.hasSpecialMessage = false;
        }
       ...
       if (set.ICE != null) {
        ...
        if (set.BUFF_VS_NECRO != null) {
            this.settings.buffVsNecro = set.BUFF_VS_NECRO;
        }
        if (set.BUFF_VS_SMASH != null) { 
            this.settings.buffVsSmash = set.BUFF_VS_SMASH; 
        }
        ...
        if (set.IS_EVIS != null) { 
            this.settings.moreToAcceleration = set.IS_EVIS; 
        }
        if (set.STAT_NAMES != null) { 
            this.settings.skillNames = set.STAT_NAMES; 
        }
        if (set.EXTRA_DAMAGE_WITH_VELOCITY != null) {
          this.settings.extraSpicy = set.EXTRA_DAMAGE_WITH_VELOCITY;
        }
        if (set.AI != null) { 
        ...
        if (set.LEVEL != null) {
            ...
            while (set.LEVEL == 'MAX' && this.skill.level < c.SKILL_CAP) {
              this.skill.score += this.skill.levelScore;
                this.skill.maintain();
            }
    ...
    refreshBodyAttributes() {
        ...
        if (this.settings.reloadToAcceleration) this.acceleration *= this.skill.acl + this.skill.mob - 1;
        if (this.settings.moreToAcceleration) this.acceleration *= Math.sqrt((this.skill.shi + this.skill.rgn)/3);
    ...
    upgrade(number) {
        if (number < this.upgrades.length && this.skill.level >= this.upgrades[number].level) {     
            ...
            this.upgrades = [];
            this.define(saveMe);
            if (this.settings.hasSpecialMessage) {
                this.sendMessage(this.settings.specialMessage);
            }
    ...
    face() {
        ...
        case 'looseWithMotion':
            ...
            break;
        case 'sideways':
            this.facing = this.velocity.direction + (Math.PI/2); 
            break;
        case 'withTarget': 
        case 'toTarget': 
    ...
    contemplationOfMortality() {
        if (this.invuln || this.invulnerable) {
            this.damageRecieved = 0;
            return 0;
        }
        // Felipe
        ...
        if (felipe  && !this.felipe && !this.felipeBullet) {
          this.kill();
        }
        if (this.felipe && this.control.alt){
          this.kill();
        }
        ...
        if (this.settings.diesAtLowSpeed) {
            if (!this.collisionArray.length && this.velocity.length < this.topSpeed / 2) {
                this.health.amount -= this.health.getDamage(5 / roomSpeed);
            }
        }
        if (this.settings.rightClickToDie && this.control.alt) {
          if (this.label == 'Booster') {
            let text = ran.choose(['became too toxic', 'accelerated to light speed', 'went too fast', 'had too much Taco Bell'])
            sockets.broadcast(this.name + ' ' + text + '.')
          }
          this.destroy();
        }
        let helluva = Math.max((this.skill.score / 10000), 1);
        if (this.settings.diesOfHunger) {
          this.range = Math.max((this.range -2 / roomSpeed), 0);
          if (!this.collisionArray.length && this.velocity.length < this.topSpeed / 2 && this.range < 1) {
                this.health.amount -= this.health.getDamage(7 / roomSpeed / helluva);
            } else if (this.velocity.length > this.topSpeed / 2) {
              this.range += 2.5 / roomSpeed;
            }
        }
        // die out of bounds
        let zeFactor = 0.5;
        let maxFactor = 1 + zeFactor;
        let minFactor = zeFactor *-1;
        if (this.x > Math.max(room.width * (maxFactor + 0.02), 410) || this.x < Math.min(room.width * (minFactor - 0.02), -210) || this.y > Math.max(room.height * (maxFactor + 0.02), 410) || this.y < Math.min(room.height * (minFactor - 0.02), -210)) {
          this.kill();
        }
        if (this.x > Math.max(room.width * maxFactor, 400)) {
          this.x = Math.min(room.width * minFactor, -200) + 10;
        }
        if (this.x < Math.min(room.width * minFactor, -200)) {
          this.x = Math.max(room.width * maxFactor, 400) - 10;
        }
        if (this.y > Math.max(room.height * maxFactor, 400)) {
          this.y = Math.min(room.height * minFactor, -200) + 10;
        }
        if (this.y < Math.min(room.height * minFactor, -200)) {
          this.y = Math.max(room.height * maxFactor, 400) - 10;
        }
        
        // life-limiting, but not to the chaser
        if (this.settings.boostInLongChase){
          if (this.velocity.length > this.topSpeed / 3) {
            this.chaseTime += 1 / roomSpeed;
          } else if (this.velocity.length < this.topSpeed / 3) {
            this.chaseTime = Math.max(this.settings.chaseTimeStart * (helluva / 3), this.chaseTime - 10 / roomSpeed);
          }
          this.penetration = this.PENETRATION + Math.max(this.velocity.length - this.topSpeed, 0) / 4;
          if (this.chaseTime > 100 * (helluva / 2) && this.velocity.length < this.topSpeed * 5) {
            this.velocity.x *= 1.2;
            this.velocity.y *= 1.2;
          }
          if (this.chaseTime > 107 * (helluva / 2)) {
            this.chaseTime = this.settings.chaseTimeStart * (helluva / 2);
            this.velocity.x /= 2;
            this.velocity.y /= 2;
          }
        }
        ...
        if (this.settings.extraSpicy != 0 && this.settings.extraSpicy != null && this.velocity.length > 1) {
          this.damage = this.DAMAGE * Math.min((this.settings.extraSpicy * this.velocity.length) / 5, 1);
        }
        ...
        if (this.isDead()) {
            ...
            if (this.anicepairofsmashers == true) {
              let o = new Entity({
                x: this.x, y: this.y
              }, this.master)
              o.define(Class.animet)
              o.velocity.x = this.velocity.x * 1.0000002;
              o.velocity.y = this.velocity.y * 1.0000002; 
              o.SIZE = this.SIZE;
            }
        ...
        connect: (() => {
            ...
            function incoming(message, socket) {
                ...
                case 'L': { // level up cheat
                    ...
                    // cheatingbois
                    if (player.body != null) { if (player.body.skill.level < c.SKILL_CHEAT_CAP || ((socket.key === process.env.SECRET) && player.body.skill.level < 25)) {
                ...
                case 'K': { // s o m e t h i n g
                    if (m.length !== 0) { socket.kick('Ill-sized testbed request.'); return 1; }
                    // cheatingbois
                    if (player.body != null) { if (player.body.label == 'Twin Smasher') {
                        player.body.define(Class.twinsmashtiddyremoval);
                    } }
                } break;
...
var gameloop = (() => {
    ...
    let collide = (() => {
            ...
            if (component >= diff.length - combinedRadius) { // A simple check
                ...
                if (goahead) {
                    ...
                    if (doDamage) {
                        ...
                        /********** DO DAMAGE *********/
                        let bail = false;
                        if (/*my.shape === n.shape && */my.settings.isNecromancer && (n.type === 'food' || n.type === 'tank') && !n.invuln && !n.invulnerable) {
                            bail = my.necro(n);
                        } else if (/*my.shape === n.shape &&*/ n.settings.isNecromancer && (my.type === 'food' || my.type === 'tank') && !my.invuln && !my.invulnerable) {
                            bail = n.necro(my);
                        } 
                        if (!bail) {
                            ...
                            let resistDiff = my.health.resist - n.health.resist,
                                damage = {
                                    _me: 
                                        ...
                                        ((my.settings.buffVsFood && n.settings.damageType === 1) ? 3 : 1 ) * //deal 3x as much damage to food 
                                        ((my.settings.buffVsNecro && n.settings.isNecromancer && !n.settings.buffVsNecro) ? 300 : 1 ) * //deal 300x as much damage to necro (drones, not the master)
                                        ((my.settings.buffVsNecro && my.settings.isNecromancer) ? 2 : 1) * // deal 2x as much damage if drone and do bonus damage against necro
                                        ((my.settings.buffVsSmash && n.settings.reloadToAcceleration) ? Math.min(n.damage, 100) : 1 ) * //deal up to 50x as much damage to smashers
                                        my.damageMultiplier() *
                                    ...
                                    _n: 
                                        ...
                                        ((n.settings.buffVsNecro && my.settings.isNecromancer && !n.settings.buffVsNecro) ? 300 : 1 ) *
                                        ((n.settings.buffVsNecro && n.settings.isNecromancer) ? 2 : 1) * 
                                        ((n.settings.buffVsSmash && my.settings.reloadToAcceleration) ? Math.min(n.damage, 100) : 1 ) *
                                        n.damageMultiplier() *
                     ...
                     if (n.shock) {
                      ...
                      if (n.poison) { 
                        my.poisoned = true
                        my.poisonLevel = n.poisonToApply
                        my.poisonTime = 30
                        my.poisonedBy = n.master
                      }
                      if (my.poison) {
                        n.poisoned = true
                        n.poisonLevel = my.poisonToApply
                        n.poisonTime = 30
        ...
        return collision => {
            ...
            if (instance.team !== other.team || instance.settings.friendlyFire || other.settings.friendlyFire) {
    ...
    function ice(my) {
      entities.forEach(function(element) {
        ...
        if (element.iceed && element.type == 'tank') {
            ...
            if (!element.invuln && !element.invulnerable) {
    ...
    function poison(my) {
      entities.forEach(function(element) {
        ...
        if (element.poisoned && element.type == 'tank') {
            ...
            if (!element.invuln && !element.invulnerable) {
              element.health.amount -= element.health.max / (40 - element.poisonLevel)
              element.shield.amount -= element.shield.max / (25 - element.poisonLevel)
            }
            element.poisonTime -= 7
...
var maintainloop = (() => {
    ...
    function placeRoids() {
        ...
        let rockcount = room.rock.length * room.width * room.height / room.xgrid / room.ygrid / 25000 / 1.5;
        let count = 0;
      
        for (let i=Math.ceil(roidcount); i; i--) { count++; placeRoid('roid', Class.obstacle); }
        for (let i=Math.ceil(roidcount * 0.4); i; i--) { count++; placeRoid('roid', Class.babyObstacle); }
        for (let i=Math.ceil(roidcount * 0.01); i; i--) { count++; placeRoid('roid', Class.autoObstacle); }
        for (let i=Math.ceil(rockcount * 0.4); i; i--) { count++; placeRoid('rock', Class.obstacle); }
        for (let i=Math.ceil(rockcount * 0.8); i; i--) { count++; placeRoid('rock', Class.babyObstacle); }
        for (let i=Math.ceil(rockcount * 0.1); i; i--) { count++; placeRoid('rock', Class.autoObstacle); }
    ...
    let spawnBosses = (() => {
        ...
        return census => {
            let heckles = 1000;
            if (timer > 6 * heckles && ran.dice((16 * heckles) - timer)) {
                util.log('[SPAWN] Preparing to spawn... {' + timer + '}');
                timer = 0;
                let choice = [];
                switch (ran.chooseChance(20, 10, 20, 20, 1, 20, 1, 10)) {
                    case 3: 
                    ...
                    case 2: 
                        ...
                        break;
                    case 4:
                        choice = [[Class.elite_elite], 1, 'a', 'nest'];
                        sockets.broadcast('A horrible chill...');
                        break;
                    case 5:
                        choice = [[Class.bossSonckWithAi], 7, 'a', 'nest'];
                        sockets.broadcast('Impending doom approaches...');
                        timer += 7 * heckles;
                        break;
                    case 6:
                        choice = [[Class.elite_massacre], 1, 'castle', 'nest'];
                        sockets.broadcast('Omae wa mou shindeiru...');
                        break;
                    case 7:
                        choice = [[Class.skimboss], 1, 'castle', 'nest'];
                        sockets.broadcast('You suddenly crave coffee...');
                        break;
                    
                }
                boss.prepareToSpawn(...choice);
                setTimeout(boss.spawn, 60000);
  ...
  if (botSpawn == undefined) {
    ...
    return 'Class.' + botSpawnTemp
               ...
               if (bots.length < c.BOTS) {
                    ...
                    o.color = 12;
                    let tank = randomTank()
                    o.define(ran.choose([Class.basick]))
...
if ((o.skill.level == c.TIER_1 || o.skill.level == c.TIER_2 || o.skill.level == c.TIER_3 || o.skill.level == c.TIER_4) && o.upgrades.length != 0) {
                    ...
                    if (o.upgrades.length > 0) {
                      number3 = ran.randomRange(-0.5, o.upgrades.length - 0.50001);
                    };
                    number2 = Math.round(number3);
            
            
            let saveMe = o.upgrades[number2].class;  
            console.log("Tier Lvl: " + o.upgrades[number2].level + " //")
            let oldtank = o.label;
            if (o.upgrades.length != 0 && o.skill.level >= o.upgrades[number2].level) {
            o.upgrades = [];
            o.upgraded = true;
            o.define(saveMe);
            console.log(number3 + " = " + number2 + ". Class: " + oldtank + " to " + o.label + ", lvl " + o.skill.level + ". UpL: " + o.upgrades.length)
            }else {
              console.log("Upgrade Cancelled, lvl: " + o.skill.level)
            }
            let ID = o.id;
            entities.forEach(instance => {
                if (instance.settings.clearOnMasterUpgrade && instance.master.id === ID) {
    ...
    let makefood = (() => {
        ...
        function getFoodClass(level) {
            ...
            switch (level) {
                ...
                case 2: a  = Class.pumpkin; break;
                case 3: a = Class.triangle; break;
        ...
        let makeMiniNestFood = () => { // Make mininest eggs
            let spot = room.randomType('mini');
            placeNewFood(spot, 0.01 * room.width, 0, true);
        };
        ...
        return () => {
            ...
            food = entities.map(instance => {
                try {
                    if (instance.type === 'tank') {
                        ...
                        else { census.sum++; census.poly++; }
                        return instance;
                    } else if (instance.foodLevel > -1) { 
                        if (room.isIn('mini', { x: instance.x, y: instance.y, })) { censusMiniNest.sum++; censusMiniNest[instance.foodLevel]++; }
                        else { census.sum++; census.poly++; }
                        return instance;
                    }
            ...
            let maxMiniNestFood = 1 + room.maxFood * room.nestFoodAmount;
            let foodAmount = census.sum;
            let nestFoodAmount = censusNest.sum;
            let miniNestFoodAmount = censusMiniNest.sum;
            ...
            while (ran.chance(0.5 * (1 - nestFoodAmount * nestFoodAmount / maxNestFood / maxNestFood))) makeMiniNestFood();
...
setInterval(iceLoop, room.cycleSpeed * 10);  
setInterval(gameloop, room.cycleSpeed);
//setInterval(entityOnUpdateLoop, room.cycleSpeed);
